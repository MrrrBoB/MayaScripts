...
def BrokenFK():
    ...
    pConstraint1 = cmds.parentConstraint(parentControl, childCtrlGrp, mo=True, skipRotate=['x', 'y', 'z'], weight=1)[
        0]  # Translate Constraint
    pConstraint2 = cmds.parentConstraint(parentControl, childCtrlGrp, mo=True, skipTranslate=['x', 'y', 'z'], weight=1)[
        0]  # Rotate Constraint
    sContstraint = cmds.scaleConstraint(parentControl, childCtrlGrp, weight=1, mo=1)
...
def BrokenConnect(parentControl, childControl):
    childCtrlGrp = cmds.listRelatives(childControl, parent=True)[0]
    pConstraint1 = cmds.parentConstraint(parentControl, childCtrlGrp, mo=True, skipRotate=['x', 'y', 'z'], weight=1)[
        0]  # Translate Constraint
    pConstraint2 = cmds.parentConstraint(parentControl, childCtrlGrp, mo=True, skipTranslate=['x', 'y', 'z'], weight=1)[
