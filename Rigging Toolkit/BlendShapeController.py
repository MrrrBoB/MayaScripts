...
def ConnectBSNonLinear(BSTargetAttrP, BSTargetAttrN, controlTransformAttr, multiplierAmtP, multiplierAmtN):
    ...
    if 'scale' in controlTransformAttr:
        ...
        # Negative
        NegativeScalePMANode = cmds.createNode('plusMinusAverage', n=nodePrefix + '_Negative_Scale_PMA')
        cmds.connectAttr(conditionNode + '.outColor.outColorG', NegativeScalePMANode + '.input1D[0]')
        cmds.connectAttr(conditionNode + '.outColor.outColorG', NegativeScalePMANode + '.input1D[1]')
        cmds.disconnectAttr(conditionNode + '.outColor.outColorG', NegativeScalePMANode + '.input1D[0]')
        cmds.setAttr(NegativeScalePMANode + '.input1D[0]', 1)
        cmds.setAttr(NegativeScalePMANode + '.operation', 2)
        NegativeScaleMDNode = cmds.createNode('multiplyDivide', n=nodePrefix + '_Negative_Scale_MD')
        cmds.connectAttr(control + '.' + controlTransformAttr + '_Negative_Multiplier',
                         NegativeScaleMDNode + '.input2X')
        cmds.connectAttr(NegativeScalePMANode + '.output1D', NegativeScaleMDNode + '.input1X')
        cmds.connectAttr(NegativeScaleMDNode + '.outputX', BSTargetAttrN)
        # Positive
        PositiveScalePMANode = cmds.createNode('plusMinusAverage', n=nodePrefix + '_Positive_Scale_PMA')
        cmds.connectAttr(conditionNode + '.outColor.outColorR', PositiveScalePMANode + '.input1D[0]')
        cmds.setAttr(PositiveScalePMANode + '.input1D[1]', 1)
        cmds.setAttr(PositiveScalePMANode + '.operation', 2)
        PositiveScaleMDNode = cmds.createNode('multiplyDivide', n=nodePrefix + '_Positive_Scale_MD')
        cmds.connectAttr(PositiveScalePMANode + '.output1D', PositiveScaleMDNode + '.input1X')
        cmds.connectAttr(control + '.' + controlTransformAttr + '_Positive_Multiplier',
                         PositiveScaleMDNode + '.input2X')
        cmds.connectAttr(PositiveScaleMDNode + '.outputX', BSTargetAttrP)
    else:
...
BSControlWindow = cmds.window(title='Controller to Blendshape', wh=(500, 300))
...
linearRadioGrp = cmds.radioButtonGrp(l='Blendshape Connection', nrb=2, la2=['Linear', 'Positive/Negative'],
                                     changeCommand='RadioChangeCommand()', sl=0)
cmds.rowLayout(nc=2)
